...
class datadog_agent(
  ...
  exec { 'uwsgi_permissions':
    command => '/usr/bin/setfacl -m g:dd-agent:rx /var/log/adroll/uwsgi';
  }
  exec { 'uwsgi-gk_permissions':
    command => '/usr/bin/setfacl -m g:dd-agent:rx /var/log/adroll/uwsgi-gk';
  }
  exec { 'uwsgi-report_permissions':
    command => '/usr/bin/setfacl -m g:dd-agent:rx /var/log/adroll/uwsgi-report';
  }
}
